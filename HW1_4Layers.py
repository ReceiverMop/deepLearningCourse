...
So if about 57000 for the first layer - > 72 neurons 
- > 24 per shape instead of desirable 36. Thats ok. Let's roll! 
'''
...
class LogisticRegression(nn.Module):
    def __init__(self,input_size, num_classes, nNeuronsInFirstLayer):
        ...
        self.secondLayer = nn.Linear(nNeuronsInFirstLayer, num_classes)
        self.outLayer = nn.Linear(num_classes,num_classes)

    def forward(self, x):
        firstLayerOut = F.relu(self.firstLayer(x))
        secondLayerOut = F.relu(self.secondLayer(firstLayerOut))
        thirdLayerOut = F.relu(self.outLayer(secondLayerOut))
        return self.outLayer(thirdLayerOut)
