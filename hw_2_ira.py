...
class GoogLeNet(nn.Module):
    ...
    def plot_error_loss(self, figsize=(5,5)):
        plt.figure(figsize=figsize)
        plt.subplot(1,2,1)
        plt.plot(self.train_loss,label='train')
        plt.plot(self.test_loss,label='test')
        plt.xlabel('epochs', fontsize=16), plt.ylabel('loss', fontsize=16)
        plt.legend(fontsize=16), plt.title('loss', fontsize=18)
        plt.subplot(1,2,2)
        plt.plot(self.train_error,label='train')
        plt.plot(self.test_error,label='test')
        plt.xlabel('epochs', fontsize=16),plt.ylabel('error [%]', fontsize=16)
        plt.legend(fontsize=16), plt.title('error', fontsize=18)
# End of GoogleNet
...
def test_network(criterion, test_loader):
    correct = 0
    total = 0
    test_loss, correct_test = 0.0, 0.0
    for images, labels in test_loader:   
        ...
        test_loss += criterion(outputs, labels).data[0] # sum up batch loss
        pred = outputs.data.max(1)[1] # get the index of the max log-probability
        correct_test += pred.eq(labels.data.view_as(pred)).cpu().sum()
        

    print 'Test Accuracy of the model on the %d test images: %.3f %%' % (test_dataset.test_labels.__len__(), 100 *correct / float(total))
    #print 'Test Accuracy of the model on the %d test images: %.3f %%' % (test_dataset.test_labels.__len__(), 100 *correct_test / len(test_loader.dataset))
    test_loss /= len(test_loader.dataset)
    test_error = 100.*(1.0 -  correct_test / len(test_loader.dataset))
    test_loss_all.append(test_loss)
    test_error_all.append(test_error)
    return test_loss_all, test_error_all
...
for epoch in range(num_epochs):
    ...
    train_loss, train_error, correct_train = 0.0, 0.0, 0.0
    for i, (images, labels) in enumerate(train_loader):
        ...
        pred = cnn(images).data.max(1)[1]  # get the index of the max log-probability\n", #pred = outputs.data.max(1)[1] 
        correct_train += pred.eq(labels.data.view_as(pred)).cpu().sum() #correct_test += pred.eq(labels.data.view_as(pred)).cpu().sum()
    train_loss /= (i + 1)
    train_loss_all.append(train_loss)
    
    train_error = 100.*(1.0 -  correct_train / len(train_loader.dataset))
    train_error_all.append(train_error)
    cnn.eval()
    (test_loss_all, test_error_all)=test_network(criterion, test_loader)
    print ('Epoch [%d/%d],  Loss: %.4f'%(epoch+1, num_epochs, test_loss_all))
...
plt.legend(fontsize=16), plt.title('loss', fontsize=18)
plt.subplot(1,2,2)
plt.plot(train_error_all,label='train')
plt.plot(test_error_all,label='test')
plt.xlabel('epochs', fontsize=16),plt.ylabel('error [%]', fontsize=16)
plt.legend(fontsize=16), plt.title('error', fontsize=18)
#plt.show()
...
for images, labels in train_loader:
    ...
    total += labels.size(0)
    correct += (predicted.cpu() == labels.cpu()).sum()
print 'Train Accuracy of the model on the %d train images: %.3f %%' % (train_dataset.train_labels.__len__(), 100 *correct / float(total))
print 'Number of correct from the %d train images: %.3f %%' % (train_dataset.train_labels.__len__(), correct)



# Save the Trained Model
torch.save(cnn.state_dict(), 'HW2_1_batch10_200_epochs.pkl')